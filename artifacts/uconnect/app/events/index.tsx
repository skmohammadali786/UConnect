...
function EventCard({
  ...
  const isOrganizer = !!currentUserId && item.organizerId === currentUserId;
      ...
      <TouchableOpacity
        ...
        style={[
          styles.card,
          {
            ...
            borderColor: isGoing ? colors.primary + "50" : colors.border,
          },
        ]}
      >
        <View style={styles.cardHeader}>
          ...
          >
            ...
            <Text
              style={[styles.orgText, { color: colors.primary }]}
              numberOfLines={1}
            >
              {item.organizer}
            </Text>
          ...
          {!isOrganizer && isPending && (
            <View style={[styles.goingBadge, { backgroundColor: "#F59E0B20" }]}>
              ...
              <Text style={[styles.goingText, { color: "#F59E0B" }]}>
                Pending
              </Text>
            </View>
          )}
        ...
        <View style={styles.cardFooter}>
          <View style={styles.attendeeRow}>
            ...
            <Text
              style={[styles.attendeeText, { color: colors.mutedForeground }]}
            >
              {item.rsvpCount + (isGoing ? 1 : 0)} attending
            </Text>
          </View>
          <TouchableOpacity
          ...
          >
            <Text
              style={[
                ...
                { color: isGoing ? "#FFF" : colors.foreground },
              ]}
            >
              {isOrganizer
                ? "Host"
...
export default function EventsScreen() {
  ...
  const handleRSVP = async (id: string) => {
    ...
    if (event?.organizerId === user.id) {
      showError(
        "Hosts cannot attend",
        "You already manage this event as the host.",
      );
      return;
    }
    ...
    if (
      ...
      await supabase.rpc("unrsvp_event", {
        p_user_id: user.id,
        p_event_id: id,
      });
      delete updated[id];
    } else {
      await supabase.rpc("rsvp_event", {
        p_user_id: user.id,
        p_event_id: id,
        p_request_note: "",
      });
      ...
      >
        ...
        <View>
          <TypewriterText
            ...
            speed={70}
          />
          {Object.values(rsvpStates).filter((r) => r.status === "approved")
            .length > 0 && (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              >
                Be the first to create an event for your college!
              </Text>
              <TouchableOpacity
                onPress={() => router.push("/events/create")}
