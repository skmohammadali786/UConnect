...
export function FadeInView({ children, delay = 0, duration = 300, from = "bottom", distance = 20, style, ...rest }: FadeInViewProps) {
...
export function ScaleOnPress({ children, style, ...rest }: any) {
  ...
  const onPressIn = () => Animated.spring(scale, { toValue: 0.96, useNativeDriver: true, tension: 200, friction: 10 }).start();
  const onPressOut = () => Animated.spring(scale, { toValue: 1, useNativeDriver: true, tension: 200, friction: 10 }).start();

  return (
    <Animated.View onTouchStart={onPressIn} onTouchEnd={onPressOut} style={[{ transform: [{ scale }] }, style]} {...rest}>
      {children}
    </Animated.View>
  );
}
