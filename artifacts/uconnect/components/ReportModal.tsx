...
export function ReportModal({ postId, visible, onClose }: ReportModalProps) {
  ...
  React.useEffect(() => {
    if (visible) {
      ...
      Animated.parallel([
        ...
        Animated.timing(slideAnim, { toValue: 300, duration: 160, useNativeDriver: ND }),
      ]).start();
      setSelected(null);
    }
  }, [visible]);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(false);
    onClose();
    showSuccess("Report submitted", "We'll review this post shortly.");
  };

  return (
    <Modal visible={visible} transparent animationType="none" onRequestClose={onClose}>
      <Animated.View style={[styles.backdrop, { backgroundColor: "rgba(0,0,0,0.6)", opacity: backdropAnim }]}>
        <TouchableOpacity style={StyleSheet.absoluteFill} activeOpacity={1} onPress={onClose} />
      </Animated.View>
      <Animated.View
        style={[styles.sheet, { backgroundColor: colors.card, borderColor: colors.border, transform: [{ translateY: slideAnim }] }]}
      >
        ...
        <View style={styles.btnRow}>
          <TouchableOpacity onPress={onClose} style={[styles.cancelBtn, { backgroundColor: colors.secondary, borderColor: colors.border }]}>
            <Text style={[styles.cancelText, { color: colors.foreground }]}>Cancel</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={handleSubmit}
            disabled={!selected || submitting}
            style={[styles.submitBtn, { backgroundColor: selected ? "#EF4444" : colors.secondary, opacity: selected ? 1 : 0.5 }]}
          >
            <Feather name="flag" size={15} color={selected ? "#FFF" : colors.mutedForeground} />
...
const styles = StyleSheet.create({
  ...
  submitBtn: { flex: 2, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7, paddingVertical: 14, borderRadius: 12 },
